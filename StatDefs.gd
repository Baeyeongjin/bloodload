...
# 336 교착의 해법이었다.
# **1구간에서 쓸모가 끝나는 지점이 200 이다** (2026-08-26 실측). 1구간 일반몹
# 체력 21.45 를 한 대에 눕히는 게 화면 200(=21.53)이고, 처치시간은 타수를
# 올림하므로 그 위로는 1초도 안 줄어든다 — 화면 301 도 900 도 36마리에 39초로
# 같았다. 900 은 "너무 높다"가 아니라 **효과가 정확히 0인 구간을 700레벨
# 열어 둔 것**이었다(사장님: "1-1 스테이지부터 300렙 찍히는데").
const CAP_BASE := 200
# 미궁은 10구간마다 5층이 열리므로(DungeonDefs.open_floors) **1층 = 2구간**이다.
# 150/37.5 = 4배로 두면 미궁이 열린 뒤 구간 축이 영영 져서 사문화된다
# (100구간 기준 미궁 5,550 대 구간 3,712). 200/80 = 2.5배면 구간 환산 1.25배
# 프리미엄이라 두 축이 실제로 경합한다 — 100~225구간대는 미궁이, 그 위는
# 구간이 상한을 연다.
const CAP_PER_FLOOR := 200
# 10구간에서 상한 920 이 되어, tests/BalanceTest 가 "첫 보스가 열린다"로
# 정의한 요구치(TIME_BOSS x0.6 = 36초 -> 화면 917)와 맞는다. 37.5 로는
# BalanceTest 자신의 첫날 설계점(12구간·화면 706)이 상한 612 를 넘어 못 샀다.
const CAP_PER_STAGE := 80.0
