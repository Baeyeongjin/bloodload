...
#
# **0.06 -> 0.6 확정** (2026-08-20, 사장님). 실측 근거:
#   90일 곡선  무과금 195/254/282 · 멤버십 200/269/289 (목표 150/250/300)
#   회귀 정책  안 누름 250/265 vs 누르면 282~296 — 누르는 게 확실히 이득
#   멤버십·정체 3일이 90일 296 으로 목표 300 에 사실상 도달
# 0.06 에서는 안 누름과 차이가 거의 없어 회귀가 축이 아니었다.
static var MARK_POWER := 0.6


# ── 혈흔 소비처 — 군림 각인 (2026-09-02 사장님 픽) ─────────────────────────
# 혈흔은 여태 "재화"가 아니라 **자동 곱배수**였다. 받는 즉시 배수에 더해지고
# 코드베이스 어디에도 `prestige_marks -=` 가 없었다 — 고를 것이 없으니 회귀
# 판은 누르는 버튼 하나짜리였다.
#
# 그런데 회귀는 지금 **조용히 꺼 버리는 것들**이 있다: best_stage 가 1 이 되는
# 순간 군림 다섯이 전부 닫힌다(스킬 칸 7->6 · 처형 문턱 · 3연격 광역 ·
# 방치 +4시간 · 소탕 2배). 화면의 "남는 것" 목록에도 안 적혀 있었다.
# 그걸 되사는 것이 이 상품이다 — **새 기계가 0이고 현존 구멍도 같이 막는다.**
#
# 규칙 변경이지 숫자가 아니다. 파는 것은 % 가 아니라 "회귀해도 안 꺼진다"다
# (군림에 % 금지 — MasteryDefs 머리글·EXPANSION 11장).
#
# 눈금: 총액 9 혈흔. 혈흔의 구간 가치는 로그로 붕괴해서 **마지막 10개는 팔아도
# 7구간밖에 안 잃는다**(5->15 가 +21구간인데 25->35 는 +7구간). 평생 총량이
# 35 개(도달 구간 500 에서 clamp)이므로 9 는 그 4분의 1이고, 남는 26 개가
# 여전히 대부분의 배수를 번다. 첫 회귀(5개)에 하나는 살 수 있어야 판이
# 처음부터 "고르는 판"으로 읽혀서 제일 싼 것을 1 로 뒀다.
const OFFERS := [
	{"key": "slot", "cost": 1},
	{"key": "execute", "cost": 1},
	{"key": "cleave3", "cost": 2},
...
static func offer_cost(key: String) -> int:
	for o in OFFERS:
		if str(o["key"]) == key:
			return int(o["cost"])
	return 0
...
# 산 각인에 쓴 혈흔의 합. **따로 저장하지 않는다** — 산 목록에서 매번 뽑으면
# 둘이 어긋날 자리가 없다(저장키 하나로 끝난다).
static func spent(kept: Dictionary) -> int:
	var n := 0
	for o in OFFERS:
		if kept.has(str(o["key"])):
			n += int(o["cost"])
	return n
