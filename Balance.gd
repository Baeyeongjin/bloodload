...
# 쓸 때는 0.55 였고, 2026-09-10 에 전진 속도를 올리면서 **0.16** 이 됐다.
# 때리는 시간이 0.45 -> 0.84 초로 늘었으므로 **같은 세기로 몹을 더 많이 눕힌다** —
...
#
# 2026-09-10 재측정: `Main.TRAVEL_SPEED` 를 200 -> 320 으로 올렸다(사장님:
# "좀 더 역동적인 전투"). **같은 맨몸 조건에서 둘을 나란히 쟀다** — 격리 저장본을
# 비우고 각각 한 번씩:
#   속도 200 → 35마리/60초 · 실측 1.71 - 모델 1.20 = **0.51초** (08-27 과 똑같이 재현)
#   속도 320 → 44마리/60초 · 실측 1.36 - 모델 1.20 = **0.16초**
# 처치가 0.58 → 0.73 마리/초로 **26% 빨라졌다.** 설계식(마리당 주기 =
# 임팩트 + max(ENGAGE_PAUSE 0.42, 간격/속도))의 예측 28% 와 오차 안이다.
#
