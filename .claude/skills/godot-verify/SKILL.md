...
## 5-1. 그림이 이상하면 **캐시부터 의심한다**

2026-08-10: 사장님 화면에서 "피의 제단"에 감시의 눈 아이콘이 떴다. 파일도 코드도
정상이었고 `.godot/imported` 가 **한 등급씩 밀린 옛 매핑**을 붙들고 있었다
(캐시에 `sk_blood_field.png` 라는 지금 없는 이름이 남아 있었다 — 이름을 바꾼 이력).

```powershell
Remove-Item "$p\.godot\imported\*" -Force
& $g --headless --path $p --import
```

**`.godot` 는 git 에 안 올라간다 — PC 마다 따로 생긴다.** 그래서 "내 화면은 멀쩡한데
사장님 화면은 다르다"가 나온다. 파일을 맞바꾸는 식으로 고치면 그때 진짜로 어긋난다.

`tests/SkillTest.gd` 가 아이콘 20종을 원본 PNG 와 직접 대조한다(임포트를 안 거치고
디코드해서 불투명 픽셀만 비교). 캐시가 밀리면 여기서 걸린다.
