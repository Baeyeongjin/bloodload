...
## 지금 빨간 검사 — `FirstHitCheck` (2026-08-27)

**셰이더 작업과 무관하다.** 밸런스 개편이 남긴 것이다.

```
사거리 진입 -> 첫 피해   평균 0.48초  중간 0.51  최대 0.88
참고: 공격 주기 0.22초 · 스윙 길이 0.22초 · 사망 연출 0.62초
assert(wait_mean < attack_interval() * 0.8)   # 0.176초
```

주기가 **0.60 → 0.22** 로 줄었는데 문턱은 여전히 `주기 x 0.8` 이다. 그런데
`_attack_swing()` 이 `min(0.34, 주기)` 라 스윙만으로 이미 0.22초다 —
**문턱이 물리적 하한보다 낮아서 어떤 코드로도 통과할 수 없다.** 게다가 남는
0.26초는 대부분 스킬 점유(주석 기준 시간의 22%)라, 주기가 짧아진 만큼 한
주기 안에서 차지하는 비중이 커진 것이다.

고칠 때 손댈 곳은 **게임이 아니라 문턱**이다. 이 검사가 지키려던 건
"마주친 뒤에 쿨다운을 처음부터 다시 세면 안 된다" 이지 총 대기시간 예산이
아니었다. 스킬 구간을 빼고 쿨다운 잔량만 재도록 바꾸는 게 맞다.

**같이 볼 것(사장님 판단):** 주기 0.22초짜리 전투에서 몹 사이 0.48초 공백은
예전 0.60초 주기 때보다 **체감이 더 크다.** 원래 이 검사를 만든 계기가
"붙어서 한참 멈췄다가 공격"(사장님)이었으니, 수치가 아니라 손맛으로 한 번
봐야 한다. `ENGAGE_PAUSE` 가 후보다.

**돌리는 데 5~9분.** 45초를 실시간으로 굴려서 그렇다. 타임아웃 200초로는
자주 못 끝낸다 — 560초를 준다.
