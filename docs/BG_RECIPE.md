...
## 6~10막 (2026-08-27) — 막을 5개에서 10개로

사장님이 A/B 후보에서 골랐다. 배경 픽: 6A · 7B · 8A · 9B · 10A.

| 막 | 파일 | 실측 지면(원본 320줄 기준) |
|---|---|---|
| 6 봉인된 심연 | `wide_abyss.png` | 175 |
| 7 달빛 유적 | `wide_ruins.png` | 234 |
| 8 혈월 제단 | `wide_altar.png` | 213 |
| 9 잠긴 습지 | `wide_marsh.png` | 209 |
| 10 핏빛 왕좌 | `wide_throne.png` | 240 |

다섯 다 45~80% 창 안이라 `fit_ground` 가 한 번에 잡았다 — 지뢰 2 재발 없음.

**밟은 지뢰 하나**: 8~10막을 넣을 때 `make_seamless` 를 다시 돌렸는데, 그건
`.orig.png` 에서 재생성하는 도구라 6·7막이 320줄로 되돌아갈 뻔했다(지뢰 1-1).
이미 이어진 것은 skip 하도록 돼 있어 무사했지만, **여러 막을 나눠 넣을 때는
넣을 때마다 전 배경의 크기를 다시 확인할 것.**

**검사가 잡은 것**: `CombatRulesTest` 가 막 수 5를 하드코딩하고 있었다
(`act_of(1) == act_of(51)` — 51 은 5막x10+1). 표에서 읽도록 고쳤다.
