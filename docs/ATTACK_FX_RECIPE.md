...
**특수 사거리 실측**(2026-08-27, `reach() = _size()*0.5 + 80` 에 배수):

| 보스 | 평상 | 특수 | 화면 폭(576) 대비 |
|---|---|---|---|
| 촉수 eye_mass | 147 | **412** | 72% |
| 음파 bone_choir | 145 | **464** | 81% |
| 처형 dark_knight | 143 | 243 | 42% |
| 유령 wraith_knight | 136 | 231 | 40% |

착지 그림은 전부 100px 남짓이다. 촉수·음파는 **그림의 네 배까지** 때린다.

**촉수만 먼저 고쳤다**(`Main._lash_trail`): 같은 크기의 덩굴을 보스 쪽으로
둘 더 세우되 뒤로 갈수록 작고 어둡고 늦게. 천장 150px 을 박아 두었다 —
사거리를 그대로 따라가면 음파(464)는 화면을 덮는다.

**전조 고리는 이미 사거리를 말하고 있다**(`Foe._special_tell` 이 `reach()`
그대로 그린다). 그러니 이 고침은 없던 정보를 주는 게 아니라 **때리는 순간에
같은 말을 반복**하는 것이다 — 예고를 놓친 플레이어가 맞고 나서 "저만큼
위험했구나"를 읽는다.
