...
### 남은 것 — 진 에픽·레전더리는 **장판이 아니라 가구다**

에픽은 **제단**, 레전더리는 **왕좌**다. 바닥 원반 위에 물건이 **서 있어서**
"땅에 깔린 장판"으로 안 읽힌다. 커먼(비명 얼굴)·언커먼(원형 문양)·레어(눈)는
납작하게 누워 있어 제대로 읽힌다 — 그래서 5종이 아니라 **2종 문제**다.

제단은 회보라색 돌이라 핏빛 계열에서도 벗어나 있다. 다시 뽑는다면 5-4 대로
**색을 말로 지정하지 말고 아이콘을 물려서** 뽑는다.

> 원인은 `FX_OVERRIDE` 의 `"field_epic"·"field_legend": {"style": "rise"}` 다 —
> `rise` 는 땅에서 **솟아오르는** 스타일이라 세워진 물건에 맞는다. 그림을 눕히면
> `hold` 로 되돌릴 수 있다. **코드가 아니라 그림이 정하는 것이다.**
...
`fx_sk_uncommon_field`(갈라진 대지) **0.435 만 낮다** — 설치본이 원본과 꽤 다르다는
뜻이다. 다시 뽑을 때 이 하나는 원본부터 다시 확인한다.
