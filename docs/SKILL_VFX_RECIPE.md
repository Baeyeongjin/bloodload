...
| `fx_sk_common_field` | `b84e2769-f30e-43c7-81a0-5607d0ec6953` | 0.992 |
| `fx_sk_uncommon_field` | `cb1334a9-5b35-4042-8746-550fc6d931d7` | **0.435** |
...
`fx_sk_uncommon_field`(갈라진 대지) 만 0.435 로 낮은데 **다른 그림이어서가 아니다.**
확대해 나란히 놓고 보니 균열 무늬가 같고 **채움 색만** 다르다(설치본은 어두운 판,
원본은 붉은 흙색). 점수식이 색 거리를 반이나 반영해서 내려간 값이다.
**설치본 쪽이 게임 화면에서 더 낫다** — 어두운 판이 "갈라진 땅"으로 읽힌다.
낮은 점수를 불량으로 읽지 말 것.

안 쓰인 후보 셋: `303aaf14`(방패), `def2a81f`(돔 방벽), `e78701f6`(심연의 손 — 다른 판).
...
#### 다단히트는 **진(field) 하나로 둔다** — 2026-08-06 사장님 결정

뱀(파 에픽)에 다단히트를 붙일지 검토하고 **안 붙이기로 했다.** 사장님: "등급은
위력만 올려."

`SkillDefs.SHAPES` 머리에 적힌 **"규칙은 형태가 정하고 등급은 위력만 올린다"**를
지킨다. 뱀만 다단히트로 만들면 등급이 규칙을 바꾸는 첫 사례가 된다.

기술적으로는 쉬웠다는 점을 적어 둔다 — `_start_field` 의 틱 구조(지속시간 ×
`tick_rate`, 틱마다 표적 재선택)가 이미 일반화돼 있어 `wave` 에 몇 줄이면 붙고,
`dps()` 가 격·파·진을 안 세므로 오프라인 모델도 안 흔들린다. **막은 것은 비용이
아니라 규칙이다.** 다시 꺼내려면 그 규칙부터 고쳐 적어야 한다.
