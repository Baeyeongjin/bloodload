...
> **다단히트를 붙이려면 여기부터다.** 지금 다단히트는 진(field) 하나뿐이고, 이유는
> "격·파는 한 번 터지는 그림이라 연타를 못 표현한다"였다. 뱀(파 레전더리)처럼
> **여러 번 파고드는 동작**을 프레임에 넣으면 그 전제가 깨진다 — 그때는 아트가 먼저고
> 코드는 `_start_field` 의 틱 구조를 그대로 쓰면 된다(이미 일반화돼 있다).
