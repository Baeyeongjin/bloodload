...
**축이 하나인 회전도 코드다** (2026-08-11, 붉은 소용돌이). 소용돌이·토네이도는
생성에 맡기면 프레임마다 모양이 흔들린다. 밝은 띠를 **한 주기만큼 아래로 흘리면**
나선이 내려가는 것이 곧 회전이다.

- **실루엣은 원본으로 마스크한다.** 안 하면 깔때기가 프레임마다 부풀었다 줄었다 한다
- **입구(맨 윗줄들)는 안 움직인다.** 같이 흘렸더니 위 테두리의 밝은 고리가 사라져서
  깔때기인지 안 읽혔다
- **가로로 돌리지 말 것.** 행마다 픽셀을 순환시켜 봤더니 띠 사이 빈 자리가 같이 돌아
  실루엣이 사다리꼴 덩어리가 됐다(실측)

**씨앗에서 빼고 싶은 부분은 잘라서 준다.** "꼬리를 빼 달라"를 프롬프트로 적으면
부정문 함정(5-3)에 걸린다. 그 부분을 지운 판을 `init_image` 로 주면 안 그린다.

**`init_image_base64` 상한은 고정값이 아니다.** 2488자가 잘려 실패했고 앞서
2752자는 통과했다. 큰 판은 **알파를 따로 두고 RGB 만 8색으로 줄여** 보낸다
(RGBA 째로 `quantize` 하면 어두운 색이 투명으로 뭉개진다 — 실측).
