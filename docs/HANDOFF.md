...
### 5-5. 남은 것 — 체력회복이 너무 세다

`hero_regen_per_sec = max_hp × 0.015 × (Lv−1)` 이라 **레벨당 최대 체력의 1.5%가
곱이 아니라 누적**된다. Lv67 이면 초당 100% 회복이라 그 뒤로는 무적이다.
`survival_seconds` 가 INF 를 돌려주므로 오프라인 판정도 생존을 아예 안 본다.

STATS 2-1 은 "+1.5%/초"라고만 적혀 있어 의도가 "레벨당 +1.5%p 누적"인지
"총 1.5%"인지 문서만으로는 못 정한다. **상한을 걸든(예: 최대 20%/초) 곡선을
바꾸든 결정이 필요하다.** 이번 재설계에서는 손대지 않았다 — 건드리면 위 표의
생존 칸이 통째로 바뀌는데, 그건 별도 검산이 필요하다.
