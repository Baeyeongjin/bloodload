...
# FOE_GAP / TRAVEL_SPEED = 0.5초가 한 마리당 달리는 시간이다.
#
# 200 -> 320 (2026-09-10 사장님: "좀 더 역동적인 전투"). **이 값은 수입의
# 지렛대다** — 마리당 주기가 `임팩트 + max(ENGAGE_PAUSE 0.42, 간격/속도)` 라
# 이동(0.80)이 대기(0.42)를 이기고 있었다. 0.50 으로 줄면 주기가 28% 짧아지고
# 그만큼 수입이 는다. 사장님이 곡선을 밀기로 하고 고른 값이다.
#
# **381 위로는 올려도 소용없다**(160/0.42). 대기가 바닥이라 페이스는 그대로인데
# 달리기 8프레임 한 바퀴가 0.42초 안에 끝나서 발만 겉돈다 — DASH_SPEED 를
# 520 으로 잡았다가 순간이동으로 보인 것과 같은 함정이다.
#
# 이 값을 바꾸면 `Balance.APPROACH_SECONDS` 를 **EngageCheck 로 다시 잰다.**
# 짐작으로 못 맞추는 값이다(0.55 -> 0.89 -> 0.76 -> 0.83 -> 0.57 -> 0.47 -> …).
const TRAVEL_SPEED := 320.0
