...
func _init() -> void:
	...
	for i in _marks(days):
		...
		print("%-8d %-8d %-8d %-10d %-10d %-6d %-8d %-10s %s"
			...
			_n(float(d["power"]))])
		print("CUMGOLD %d earned=%.6e power=%.6e lv_dmg=%d cap=%d stage=%d" % [i, float(d.get("earned",0.0)), float(d["power"]), int(d["lv"]), int(d["cap"]), int(d["stage"])])
	print("")
	...
	print("AXES_DUMP " + JSON.stringify(free["dump"]))
	print("PaceProbe OK")
	quit()
...
func _run(days: int, member: bool, prestige_after: int = 1) -> Dictionary:
	...
	for day in days:
		...
		var _e1 := _farm(game, idle * 3600.0, 0.5)
		gold += _e1
		earned += _e1
		# 2) 접속 — 그동안 계속 민다. 미는 도중에도 버는 게 실제 흐름이라
		#    구간을 넘길 때마다 그 구간의 몫이 들어온다.
		var _e2 := _farm(game, PLAY_MINUTES * 60.0, 1.0)
		gold += _e2
		earned += _e2
		...
		diag.append({"stage": game.stage, "floor": game.dungeon_best,
			...
			"gold": gold, "crystal": game.crystal, "earned": earned,
			"power": Balance.combat_power(game.dps(), game.max_hp(),
				game.regen_per_sec())})
	var dump := {}
	for st in StatDefs.STATS:
		dump[str(st["key"])] = int(game.stat_lv(str(st["key"])))
	dump["_eff_damage"] = float(game._stat_eff("damage"))
	dump["_eff_tough"] = float(game._stat_eff("tough"))
	dump["_eff_speed"] = float(game._stat_eff("speed"))
	dump["_eff_crit"] = float(game._stat_eff("crit"))
	dump["_eff_critdmg"] = float(game._stat_eff("critdmg"))
	dump["_eff_regen"] = float(game._stat_eff("regen"))
	dump["_gear_damage"] = float(game._gear_stat("damage"))
	dump["_gear_armor"] = float(game._gear_stat("tough"))
	dump["_hero_lv"] = int(game.hero_lv)
	dump["_dps"] = float(game.dps())
	dump["_hp"] = float(game.max_hp())
	dump["_regen"] = float(game.regen_per_sec())
	dump["_interval"] = float(game.attack_interval())
	dump["_cap"] = int(StatDefs.train_cap(game.dungeon_best, game.best_stage))
	dump["_earned"] = earned
	return {"log": log, "peak": peak, "diag": diag, "axes": _axes(game), "dump": dump,
