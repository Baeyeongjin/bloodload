...
#
# **세로 208 = 전투 띠(416)의 절반.** 160 이었을 때는 그림이 띠를 못 덮어서 위는
# 하늘 그라데이션, 아래는 담을 뒤집어 잇고 검게 페이드해서 메웠다 — 그래도 화면
# 위쪽에 판때기 같은 띠가 남았다(사장님: "그 위에도 배경이 채워져 있어야 하지 않니").
# 208 로 다시 뽑으니 그림 하나가 띠를 통째로 덮어서 그 보정이 전부 필요 없어졌다.
const BG_SRC := Vector2i(744, 208)
