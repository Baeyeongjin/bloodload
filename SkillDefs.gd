...
const RULES := {
	...
	# **문양은 무리 가운데 하나다**(2026-08-10 사장님: "각몬스터 밑에있으면 너무
	# 지저분해보임"). 비명의 흔적과 같은 규칙(puddle)이다 — 96px 그림 x2.0 = 192px.
	# 2.5(240px)까지 늘려 봤는데 잉크가 옆으로 퍼져 얇아졌다. 덮는 폭보다 **밀도**가
	# 균열로 읽히게 한다.
	# 2026-08-10 후보 셋을 배경 위에 얹어 보고 사장님이 **B(솟구치는 피)** 를 골랐다.
	# 앞의 균열 그림은 돌바닥이 딸려 와 사각형 덩어리로 보였다(두 판 다 그랬다).
	#
	# 배율 2.2 — 사장님: "비명의 흔적처럼 이팩트 크기를 좀 늘려줘".
	# 96px x 등급 1.12 x 2.2 = 237px 로 비명의 흔적(64 x 3.0 = 192px)보다 조금 넓다.
	# 1.4(150px)로 뒀을 때는 세 칸(320px)에 비해 작아 보였다.
	"field_uncommon": {"as": "wave", "pit_kill": true, "quake": 7.0,
		"max_targets": 3, "fps": 12.0, "puddle": 2.2},
