...
const RULES := {
	...
	#
	# **배율 1.35 는 폭 문제 때문에 내린 값이다.** 영웅과 첫 몹이 72px 밖에 안 떨어져
	# 있어서(영웅 213 / 몹 285), 2.2(237px)로 키우니 발밑에 정확히 놓고도 영웅을 통째로
	# 덮어 "발밑이 아니다"로 보였다. 잉크는 캔버스 가운데에 있으므로(어긋남 2px)
	# 자리가 아니라 폭이 원인이다.
	# 96 x 등급 1.12 x 1.35 = 145px -> 285 중심으로 212~357. 몹 몸통(256~314)을 덮고
	# 영웅 몸통(181~245)과는 끝만 겹친다. **여기서 더 키우면 다시 영웅을 먹는다.**
	"field_uncommon": {"as": "wave", "pit_kill": true, "quake": 7.0,
		"max_targets": 3, "fps": 12.0, "puddle": 1.35},
