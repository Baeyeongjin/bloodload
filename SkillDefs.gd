...
const RULES := {
	...
	"field_uncommon": {"as": "wave", "pit_kill": true, "quake": 7.0,
		"max_targets": 3, "fps": 12.0, "puddle": 2.0},
...
const FX_OVERRIDE := {
	...
	# 처형자의 아가리 — 그림은 **정면으로 벌린 입**이라 그대로 두면 화면 밖(관객)을
	# 문다. 표적 쪽으로 기울여 "저놈을 문다"로 읽히게 한다(2026-08-10 사장님 지시).
	# 세게 돌리면 정면 대칭 그림이 찌그러지므로 기울이기(skew)는 계속 끈다.
	"strike_epic": {"skew": 0.0, "rot": -28.0},
