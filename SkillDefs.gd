...
const RULES := {
	...
	#
	# **위력과 쿨타임을 같이 옮긴다.** 격 값(2.2 / 7초)으로 둘을 때리면 쿨타임이 같은
	# 사혈 발톱(레어)보다 세진다 — RULES.hits 주석과 같은 함정이다.
	#
	# 다만 **위력을 파 값(1.4)까지 내리면 승급이 약해진다**: 1.4 x 1.35 = 1.89 로
	# 커먼(2.2 x 1.0 = 2.2)보다 낮아서, 피의 송곳니를 승급하면 손해다.
	# `tests/SkillTest` 의 "승급했는데 안 세다"가 그걸 잡았다 — 잡히지 않았으면
	# 뽑기 보상이 조용히 뒤집혀 있었을 것이다.
	#
	# 그래서 **위력은 승급이 이기는 최소선(1.75 -> 2.36 > 2.2)까지만 내리고,
	# 균형은 쿨타임으로 치른다**(7 -> 13초, 파 값). 한 방은 세지고 초당은 낮아진다.
	"strike_uncommon": {"as": "wave", "pierce": true, "max_targets": 2,
		"power": 1.75, "cooldown": 13.0},
	...
	# ── 신화 4종 (2026-09-02 사장님: "신화 고유 규칙도 ㄱㄱ") ────────────────
	# 넷 다 **피해로는 못 만드는 결과**다 — 처형과 같은 원칙.
	# 혈신의 송곳니 — **처치하면 쿨다운이 그 자리에서 돌아온다.** 한 입에
	# 죽이면 연달아 문다. 잡몹 줄이 이빨 앞에서 무너지는 그림이다.
	"strike_mythic": {"reset_on_kill": true},
	# 핏빛 해일 — **몹을 뒤로 쓸어낸다**(보스·중간보스는 안 밀린다 — 보스전
	# 시간 설계가 무너진다). 피해가 아니라 **시간을 버는** 규칙이다.
	"wave_mythic": {"puddle": 1.0, "knockback": 64.0},
	# 혈계 강림 — **장판 틱 피해의 일부를 체력으로 마신다.** 혈귀가 제 이름값을
	# 하는 자리다. 혈액(재화)이 아니라 체력이다 — "전투가 돈을 준다" 예외를
	# 만들지 않는다(_strike_once 의 흡혈 삭제 주석과 같은 결).
	"field_mythic": {"lifesteal": 0.25},
	# 영겁의 성혈 — **치명상을 한 번 무르고 완전 회복한다.** 부활 뒤 긴 내부
	# 쿨(초). 상위 가호들처럼 패시브다 — 끼고 있는 것 자체가 계약이다.
	"ward_mythic": {"passive": true, "revive": 300.0},
	# 피의 왕좌 — `fixed` 로 **전진해도 안 밀린다**(2026-08-11 사장님: "바닥에 고정만").
	# 왕좌는 세워 놓는 물건이라 뒤로 흘러가면 "저기 두고 온 가구"가 된다.
	# 그림은 그대로다 — 다시 안 뽑는다.
	# `execute` — **문양 안에서 체력 15% 아래로 떨어진 놈은 즉사한다**(2026-08-11
