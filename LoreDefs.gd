...
# 걸면 안 읽는 사람이 벌을 받는다. 값이 작은 건 막 하나당이라서다.
#
# 막이 다섯에서 열로 늘었을 때(10막 확장) 여기를 안 채웠다 — LoreCheck 가
# "연대기 줄 수(5)가 막 수(10)와 다르다"로 계속 빨간 상태였다.
...
const CHRONICLE := [
	...
	"심연에는 바닥이 없었다. 떨어지는 동안 자란 것들이 우리를 마중 나왔다.",
	"유적은 달을 기다린다. 뿌리가 돌을 감고, 돌은 그것을 견디기로 했다.",
	"제단은 달이 붉을 때만 입을 연다. 바친 것과 바쳐진 것이 자리를 바꾼다.",
	"늪은 아무것도 돌려주지 않는다. 가라앉은 왕관이 아직 명령을 내리고 있다.",
	"왕좌를 빼앗는 자는 왕좌가 된다. 그 자리는 늘 누군가를 앉히고 기다렸다.",
]
