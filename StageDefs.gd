...
# 제한 시간이 붙어도 구간이 순식간에 끝나 시계를 볼 일이 없었다.
# 이 숫자는 "칸이 비면 바로 채운다"(Main._refill_lanes)와 한 몸이다. 무리 단위로
# 끊어 보내면 걸어 들어오는 시간만 100마리 x 2.3초라 애초에 못 넘는다.
#
# **TIME_NORMAL 과 같이 움직여야 한다.** 한 마리당 다음 몹에게 달려가는 시간이
# 0.55초(Balance.APPROACH_SECONDS) 고정이라, 100마리면 피해를 0초에 넣어도 55초가
# 그냥 나간다 — 60초 제한에 100마리를 얹으면 어떤 수치로도 못 넘는 구간이 된다.
...
# 일반 구간은 KILLS_PER_STAGE 와 같이 간다(초당 1마리). 보스·중간보스는 한 마리라
# 처치 수와 무관하다.
const TIME_NORMAL := 100.0
