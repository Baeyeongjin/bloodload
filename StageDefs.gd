...
# 전 구간 공통 배율 (2026-08-12 사장님: "지금보다 2배 더 어렵게").
# 1단계 몹 체력 7 -> 14 인데 첫 평타가 27 이라 여전히 한 방 — 신규도 안 막힌다.
static var POWER_MULT := 2.0
...
static func enemy_power(stage: int) -> float:
	return POWER_MULT * pow(POWER_STEP,
		float(maxi(1, stage) - 1) / float(STEPS_PER_STAGE))
