...
static var POWER_STEP := 1.40       # 큰 단계(10구간)마다 적이 세지는 배수
...
static func gold_per_kill(stage: int) -> float:
	var p := float(maxi(1, stage) - 1) / float(STEPS_PER_STAGE)
	return (1.0 + GOLD_SLOPE * p) * pow(GOLD_STEP, p)
