...
#
# 100 -> 40 (2026-08-06, 사장님). 100 은 "초당 1마리 x 제한 100초"에서 나온 값인데
# 제한이 없어졌으니 근거가 남지 않았고, 실제로는 120초가 걸려 늘어졌다.
# 40마리면 약 46초 — 레퍼런스 방치형의 일반 구간(40~60초) 대역이다.
#
# 이 숫자는 "칸이 비면 바로 채운다"(Main._refill_lanes)와 한 몸이다. 무리 단위로
# 끊어 보내면 걸어 들어오는 시간만 40마리 x 2.3초라 하염없이 늘어진다.
const KILLS_PER_STAGE := 40
...
# 마다 이 값을 넘는지 본다.
#
# **55 는 실측에서 나온 값이다** — 한 마리당 1.15초(1-1 맨몸 60초 = 52마리)이므로
# 40마리면 46초, 여유 20%. 레퍼런스 방치형의 일반 구간 대역(40~60초)에 맞춘다.
# 레버는 KILLS_PER_STAGE 하나뿐이다(그 주석 참고).
const PACE_NORMAL := 55.0
