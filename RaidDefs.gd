...
#
# 대기는 간격보다 **작아야 한다** — 일부러다. 이래야 위 max 에서 간격이 이기고,
# "겹치게 세우면 빨라진다"가 실제로 성립한다.
#
# 0.18 -> 0.12 (2026-09-10). `Main.TRAVEL_SPEED` 가 200 -> 320 이 되면서 간격이
# 50/200 = 0.25 에서 50/320 = **0.156** 으로 줄어, 옛 0.18 이 그걸 이겨 버렸다 —
# 지렛대가 조용히 뒤집힌 것이다(RaidPaceCheck 가 잡았다: "간격을 좁혀도 아무
# 일도 안 일어난다"). **전진 속도를 만지면 이 값도 같이 봐야 한다.**
# 마리당 0.258 + 0.156 = 0.414초 -> 60초에 **145마리**(요구 100, 여유 45%).
#
# 본편을 안 건드리는 이유: 그 박자에 **수입 곡선이 걸려 있다**(마리당 시간이
# 곧 혈액 시급이다). 재화 던전은 처치가 아니라 격파 뭉치로 주므로 안 걸린다.
const SWARM_PAUSE := 0.12
