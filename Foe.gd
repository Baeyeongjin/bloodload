...
# 몸통 **세로** 가운데(화면 y). 원점이 발밑이라 그림 높이의 절반만큼 위다.
#
# **가로 반폭(`body_half`)을 세로 기준으로 쓰면 안 된다.** 이펙트 자리가 그렇게 잡혀
# 있었고, 몹 비율에 따라 어긋났다(2026-08-06 실측): 아이스 위습(홀쭉)은 이펙트가
# 몸 밖 위로 나가고, 서리 거미(넓적)는 정수리에 붙었다. 우연히 비슷해 보였던 것은
# 잡몹 대부분이 비슷한 비율이라서다.
func body_mid_y() -> float:
...
func _draw() -> void:
	...
	if is_boss or is_midboss:
		hit_f *= 0.5
	# **아주 얕게 눌린다**(사장님 2026-08-25: 3% / 2.5%). 도트는 배율이
	# 정수가 아니면 늘린 만큼 픽셀이 깨진다 — 옛 12%/10% 는 그 경계를
	# 넘어 "맞으면 일그러진다"로 읽혔다. 움직임만 남기고 깨짐은 버린다.
	wsc *= 1.0 + 0.03 * hit_f
	hsc *= 1.0 - 0.025 * hit_f
